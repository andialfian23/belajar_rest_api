...
class HMIF extends RestController
{
    ...
    public function index_get()
    {
        ...
        if ($id === null) {
		   $mhs = $this->db->query("SELECT 
			...
			npm, 
			nama as nm_pd, 
			alamat,  homebase, kode_prodi, kode_fak
			telp as no_hp,  
			email 
			FROM t_anggota 
            ...
            $mhs = $this->db->query("SELECT id_pmb as id_mhs, 
				...
				nama as nm_pd, 
				alamat, telp as no_hp,  email, homebase, kode_prodi, kode_fak 
				FROM t_anggota WHERE npm='".$id."'")->row_array();
        }
